...
let rec map f seq () =
  match seq () with
  | Cons (el, seq) -> Cons (f el, map f seq)
  | Nil -> Nil

let rec flatten seq () =
  match seq () with
  | Cons (inner_seq, seq) ->
    flatten_and_append inner_seq seq ()
  | Nil -> Nil
and flatten_and_append seq tail () =
  match seq () with
  | Cons (el, seq) ->
    Cons (el, flatten_and_append seq tail)
  | Nil -> flatten tail ()

let rec flat_map f seq () =
  match seq () with
  | Cons (el, seq) ->
    flat_map_append f (f el) seq ()
  | Nil -> Nil
and flat_map_append f seq tail () =
  match seq () with
  | Cons (el, seq) ->
    Cons (el, flat_map_append f seq tail)
  | Nil -> flat_map f tail ()

let rec zip seq1 seq2 () =
  match (seq1 (), seq2 ()) with
  | (Cons (el1, seq1), Cons (el2, seq2)) ->
    Cons ((el1, el2), zip seq1 seq2)
  | _ -> Nil
