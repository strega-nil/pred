...
let nth n arr = if n < length arr then Some (Caml.get arr n) else None
...
let sub start end_ arr = Caml.sub arr start (end_ - start)
...
let find p arr =
  let rec helper p arr len i =
    if i < len then
      ...
      if p x then Some (i, x) else (helper [@tailcall]) p arr len (i + 1)
    else None
  in
...
let fill start end_ el arr = Caml.fill arr start (end_ - start) el
...
let map f arr = init (length arr) (fun i -> f (unsafe_get arr i))
